...
declare global {
  ...
  namespace google.maps {
    ...
    // --- Map ---
    export class Map extends MVCObject { constructor(mapDiv: HTMLElement, opts?: MapOptions); setCenter(latlng: LatLng | LatLngLiteral): void; setZoom(zoom: number): void; fitBounds(bounds: LatLngBounds | LatLngBoundsLiteral, padding?: number): void; setOptions(options: MapOptions): void; }
    export interface MapOptions { center?: LatLng | LatLngLiteral; zoom?: number; mapTypeId?: string; mapTypeControl?: boolean; streetViewControl?: boolean; fullscreenControl?: boolean; zoomControl?: boolean; styles?: MapTypeStyle[]; }
    ...
    export interface PolylineOptions { path?: any[]; strokeColor?: string; strokeOpacity?: number; strokeWeight?: number; map?: Map; geodesic?: boolean; }
...
const GoogleMapView: React.FC<GoogleMapViewProps> = ({
  ...
  useEffect(() => {
    const init = () => {
      if (mapRef.current && window.google && window.google.maps) {
        const newMap = new window.google.maps.Map(mapRef.current, {
          center, zoom, mapTypeId, styles: styles || [], mapTypeControl: true, streetViewControl: false, fullscreenControl: false, zoomControl: true,
        });
        setMap(newMap);
   ...
   // Update map options like center, zoom, style
  useEffect(() => {
    if (map) {
      map.setCenter(center);
      map.setZoom(zoom);
      map.setOptions({ styles: styles || null, mapTypeId: mapTypeId || 'roadmap' });
    }
  }, [map, center, zoom, styles, mapTypeId]);
  ...
  useEffect(() => {
    ...
    paths.forEach(pathData => {
      const polyline = new window.google.maps.Polyline({
        path: pathData.coordinates, geodesic: pathData.geodesic !== false, // Default to true
