...
import { UploadedNagadFile } from '../types';
import { parseNagadExcelFile } from '../utils/nagadParser';

interface FileWithSourceName extends File {
  sourceName?: string;
}

const NagadFileUpload: React.FC = () => {
  const { addNagadFile, removeNagadFile, updateNagadFileSourceName, uploadedNagadFiles, setIsLoading, setError, error } = useNagadContext();
  ...
  const onDrop = useCallback(async (acceptedFiles: FileWithSourceName[]) => {
    setIsLoading(true);
    setError(null);
    let fileIndexOffset = uploadedNagadFiles.length;

    for (const file of acceptedFiles) {
      try {
        const { records, headers } = await parseNagadExcelFile(file);
        
        const fileId = uuidv4();
        const userDefinedSourceName = file.sourceName || `Nagad Statement ${fileIndexOffset + 1}`;
        fileIndexOffset++;

        if (records.length === 0 && headers.length === 0) {
             console.warn(`File ${file.name} appears empty or not a valid Nagad statement format.`);
        }
        
        const newFile: UploadedNagadFile = {
          id: fileId,
          name: file.name,
        ...
        addNagadFile(newFile);

      } catch (err) {
        console.error('Error processing Nagad file:', file.name, err);
        setError(`Error processing Nagad file ${file.name}: ${err instanceof Error ? err.message : 'Unknown error'}`);
      }
    }
    setIsLoading(false);
  }, [addNagadFile, setIsLoading, setError, uploadedNagadFiles.length]);
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    accept: {
      ...
      'text/csv': ['.csv'], // Also allow CSV for Nagad statements
    },
    multiple: true,
     getFilesFromEvent: async (event: any) => { 
      const files = Array.from(event.target.files || event.dataTransfer.files || []);
      return files.map((file: any, index: number) => Object.assign(file, {
        sourceName: `Nagad Statement ${uploadedNagadFiles.length + index + 1}` 
      }));
    }
  });
  ...
  return (
    <div className="bg-surface shadow-xl rounded-xl p-4 sm:p-6 border border-neutral-light">
      <h2 className="text-xl font-semibold text-textPrimary mb-4">Upload Nagad Statement Files</h2>
      ...
      >
        ...
        {isDragActive ? (
          <p className="text-primary-dark font-semibold">Drop Nagad files here ...</p>
        ) : (
          <p className="text-textSecondary text-sm sm:text-base">Drag 'n' drop Nagad Excel/CSV files here, or click to select</p>
        )}
      </div>
            ...
            {uploadedNagadFiles.map((file) => (
              ...
              >
                <div className="flex items-center mb-2 sm:mb-0 flex-grow min-w-0">
                  <FileText className="h-6 w-6 text-emerald-500 mr-2.5 sm:mr-3 flex-shrink-0" />
                ...
                <div className="flex items-center space-x-2 flex-shrink-0 self-end sm:self-center mt-1 sm:mt-0">
                  ...
                  <button onClick={() => removeNagadFile(file.id)} title="Remove file"
...
export default NagadFileUpload;
