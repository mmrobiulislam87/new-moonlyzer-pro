...
import { ChevronDown, ChevronUp, FileText as FileIcon, Filter as FilterIcon, Download, Info, AlertTriangle, XCircle, Globe, Activity, PackageOpen, Smartphone, BrainCircuit } from 'lucide-react';
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-4">
        <h3 className="text-xl font-semibold text-textPrimary">
          IPDR Records ({sortedRecords.length})
        </h3>
            ...
            <table className="min-w-full divide-y divide-neutral-light">
              ...
              <tbody className="bg-surface divide-y divide-neutral-light">
                {currentTableData.map((record, index) => (
                  <tr key={record.id} className="transition-colors hover:bg-primary-lighter/20">
                      {uniqueHeaders.map(rawHeaderKey => {
                        const mappedKey = (EXPECTED_IPDR_HEADERS_MAP_LOCAL[rawHeaderKey] || 
                                          rawHeaderKey.toLowerCase().replace(/\s+/g, '_').replace(/[^\w_]/gi, '')
                                          ) as keyof IPDRRecord;
...
const IPDRDataView: React.FC = () => {
    ...
    if (uploadedIPDRFiles.length === 0 && !isLoading) {
        return (
            <div className="flex items-center justify-center h-full">
                <div className="flex flex-col items-center justify-center text-center p-8 sm:p-12 bg-surface rounded-2xl shadow-xl border border-neutral-light/50 min-h-[450px] w-full max-w-4xl">
                    <Globe size={52} className="text-accent mb-6" />
                    <h2 className="text-2xl sm:text-3xl font-bold text-textPrimary mb-3">IPDR Analysis Workspace</h2>
                    <p className="text-md sm:text-lg text-textSecondary mb-8 max-w-2xl">
                        Upload your Internet Protocol Detail Records (IPDR) to analyze internet usage patterns. Track browsing behavior, application usage, link devices to IPs, and gain AI-powered insights into digital footprints.
                    </p>
                    <div className="flex flex-wrap justify-center gap-x-6 sm:gap-x-10 gap-y-4">
                        ...
                        <div className="flex flex-col items-center text-xs sm:text-sm text-orange-600 w-24 text-center">
                           <PackageOpen size={32} className="mb-1" />
                           Application Usage
                        </div>
                        <div className="flex flex-col items-center text-xs sm:text-sm text-yellow-600 w-24 text-center">
                           <Smartphone size={32} className="mb-1" />
                           Device Linkage
                        </div>
                        <div className="flex flex-col items-center text-xs sm:text-sm text-purple-500 w-24 text-center">
                           <BrainCircuit size={32} className="mb-1" />
                           AI-Powered Insights
                        </div>
                    </div>
                </div>
        ...
        <div className="space-y-4">
          {filesForTabs.length > 1 && (
             <div className="bg-surface p-3 sm:p-3.5 rounded-xl border border-neutral-light shadow-lg">
              <h3 className="text-xs sm:text-sm font-medium text-textSecondary mb-2 sm:mb-2.5 ml-1">Select IPDR File to View:</h3>
              <FileTabs>
