...
const DataTable: React.FC = () => {
  ...
  const recordsForDisplay = useMemo(() => {
    if (activeFileTabId) {
      return globallyFilteredRecords.filter(record => record.sourceFileId === activeFileTabId);
    }
    return globallyFilteredRecords;
  }, [globallyFilteredRecords, activeFileTabId]);
  ...
  const uniqueFileHeaders = useMemo(() => {
    ...
    if (activeFile) {
        ...
        if (targetFilesForHeaders.length === 0 && uploadedFiles.length > 0) {
            targetFilesForHeaders = uploadedFiles;
        }
        targetFilesForHeaders.forEach(file => file.headers.forEach(h => headersSet.add(h)));
    }

    if (headersSet.size === 0 && uploadedFiles.length > 0) {
  ...
  const sortedRecords = useMemo(() => {
    let sortableItems = [...recordsForDisplay];
  ...
  const handleExportData = () => {
    ...
    if (!activeFileTabId && uploadedFiles.filter(f => filterState.selectedFileIds.includes(f.id)).length > 1 && uniqueFileHeaders.length > 0) {
        headersToExport.push("Source File");
    }
  ...
  if (isLoading && recordsForDisplay.length === 0 && uploadedFiles.length > 0) {
  ...
  if (recordsForDisplay.length === 0 && uploadedFiles.length > 0 && (activeFileTabId || filterState.selectedFileIds.length > 0) ) {
    return (
      <div>
                  ...
                  return (
                    ...
                    >
                      {uniqueFileHeaders.map(headerKey => (
                        <td key={`${record.id}-${headerKey}`} className="px-3.5 py-2.5 whitespace-nowrap text-xs text-textSecondary">
