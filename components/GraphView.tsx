...
import { Share2, Zap, Info, ZoomIn, ZoomOut, RefreshCw, EyeOff, Eye, Edit, Download, X, Search as SearchIcon, XCircle, CodeXml, Phone, Clock, FileText, Users, Link as LinkIcon, CalendarDays, TowerControl, Route, Eraser, History, Filter as FilterIconLucide, Palette as PaletteIcon, Droplets, Smartphone as SmartphoneIcon, Landmark as BankIcon, MessageSquare as SmsIcon, Mail as MailIcon, User as UserSquareIcon, UserRound as UserRoundIcon, UserX, ShieldCheck, Skull, Truck, CarFront, Bike, CircleSlash, ChevronDown, ChevronUp, Maximize2, Minimize2, PaintBucket, Image as ImageIcon, LayoutGrid, RefreshCcw, AlertTriangle, BrainCircuit, Sparkles } from 'lucide-react';
...
import SNAInsightsPanel from './SNAInsightsPanel';
...
const GraphView: React.FC = () => {
  ...
  const { 
    ...
    globallyFilteredRecords: filteredRecords, 
    uploadedFiles,
    isGraphDataTrimmed,
    nodePositions, setNodePosition, removeNodePosition, clearAllNodePositions
  } = useCDRContext();
 ...
 const runAlgorithmicLayout = useCallback((layoutNameToRun: string, animated = true) => {
    ...
    const lockedNodes = positionedNodes.lock();

    let currentLayoutOptions: cytoscape.LayoutOptions;
    const aPartyNode = cy.$('node[?isAPartyNode]');
    const roots = aPartyNode.length > 0 ? aPartyNode : undefined;

    switch(layoutNameToRun) {
        case 'breadthfirst':
            currentLayoutOptions = { name: 'breadthfirst', directed: true, padding: 30, spacingFactor: 1.2, animate: animated, animationDuration: 500, ...(roots && roots.size() > 0 && { roots: roots.map(n => n.id()) }) };
            break;
        case 'circle':
            currentLayoutOptions = { name: 'circle', padding: 30, spacingFactor: 1.2, animate: animated, animationDuration: 500 };
            break;
        case 'grid':
            currentLayoutOptions = { name: 'grid', padding: 30, spacingFactor: 1.2, animate: animated, animationDuration: 500 };
            break;
        case 'random':
            currentLayoutOptions = { name: 'random', fit:true, padding: 30, animate: animated, animationDuration: 500 };
            break;
        case 'cose':
        default:
    ...
    cy.scratch('_currentLayoutName', layoutNameToRun);
}, [coseLayoutOptions]);
  ...
  return (
    <div className={cn("h-full flex flex-col", isGraphFullscreen ? 'fixed inset-0 z-[999] bg-background' : 'relative space-y-3')}>
      {isGraphDataTrimmed && !isGraphFullscreen && (
        <div className="p-2 bg-warning-lighter text-warning-darker rounded-lg border border-warning-light flex items-center shadow-md flex-shrink-0">
          <AlertTriangle size={16} className="mr-2"/>
          <p className="text-xs">
            Dataset is large. Graph shows a partial view of 50,000 records. Apply more filters for a complete analysis.
          </p>
        </div>
      )}

      {/* Top Controls */}
      <div className="p-3 bg-surface border border-neutral-light rounded-xl shadow-xl space-y-3 flex-shrink-0">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
            <div className="mb-2 md:mb-0">
                <div className="flex items-center text-xl font-semibold text-textPrimary mb-1"> <Share2 size={22} className="mr-2 text-primary" /> Call Network Graph </div>
                <p className="text-xs text-textSecondary"> Visualizing {visibleNodesCount} numbers and {visibleEdgesCount} visible links. <br/> <Zap size={12} className="inline text-danger-dark" /> Hubs highlighted. Node size by call volume. Right-click for options. </p>
            </div>
             <div className="w-full md:w-auto flex flex-col sm:flex-row flex-wrap gap-2 items-stretch">
                <button onClick={() => setIsInsightsPanelOpen(true)} title="AI SNA Insights" className="px-3 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600 text-sm flex items-center shadow-md"><Sparkles size={16} className="mr-1.5"/> AI SNA Insights </button>
                 <div className="flex-grow sm:flex-grow-0 flex">
                    <div className="relative flex-grow">
                        <input type="text" placeholder="Find Node (ID/Name)" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} onKeyDown={handleSearchKeyDown} className="w-full pl-3 pr-8 py-2 text-sm border border-neutral-light rounded-l-lg focus:ring-1 focus:ring-primary-light bg-surface" aria-label="Find Node in Graph" />
                        {searchTerm && <button onClick={clearSearch} className="absolute inset-y-0 right-0 pr-2 flex items-center text-neutral-DEFAULT hover:text-danger-dark" title="Clear search"><XCircle size={15} /></button>}
                    </div>
                    <button onClick={handleNodeSearch} title="Find Node" className="px-3 py-2 bg-primary text-white rounded-r-lg hover:bg-primary-dark text-sm flex items-center shadow-md"> <SearchIcon size={16}/> </button>
                </div>
                <div className="flex gap-2 items-center">
                    <select value={activeGraphLayout} onChange={(e) => setActiveGraphLayout(e.target.value)} className="p-2 text-xs border border-neutral-light rounded-lg focus:ring-1 focus:ring-primary-light shadow-sm h-full" title="Select Layout Algorithm">
                        {availableLayouts.map(layout => (
                            <option key={layout.name} value={layout.name}>{layout.label}</option>
                        ))}
                    </select>
                    <button onClick={() => runAlgorithmicLayout(activeGraphLayout, true)} title="Re-apply Current Layout" className="p-2 bg-neutral-lighter hover:bg-neutral-light rounded-lg text-primary-dark shadow-sm h-full"><RefreshCcw size={16}/></button>
                    <button onClick={clearAllNodePositions} title="Clear All Manual Positions" className="p-2 bg-neutral-lighter hover:bg-neutral-light rounded-lg text-danger shadow-sm h-full"><Eraser size={16}/></button>
                </div>
                <div className="flex gap-2 flex-wrap">
                    {hiddenNodeIds.size > 0 && <button onClick={resetHiddenNodes} title="Show All Hidden Nodes" className="p-2 bg-secondary-lighter/60 hover:bg-secondary-lighter/80 rounded-lg text-secondary-darker flex items-center text-xs shadow-sm"><Eye size={14} className="mr-1"/> Show All ({hiddenNodeIds.size}) </button>}
                    <button onClick={() => cyRef.current?.fit(undefined, 50)} title="Fit to View" className="p-2 bg-neutral-lighter hover:bg-neutral-light rounded-lg text-neutral-darker shadow-sm"><Maximize2 size={16}/></button>
                    <button onClick={downloadPNG} title="Download Graph as PNG" className="p-2 bg-success text-white rounded-lg hover:bg-success-dark flex items-center text-xs shadow-sm"><Download size={14} className="mr-1"/> PNG </button>
                    <button onClick={downloadGraphJSON} title="Download Graph as JSON" className="p-2 bg-info text-white rounded-lg hover:bg-info-dark flex items-center text-xs shadow-sm"><CodeXml size={14} className="mr-1"/> JSON </button>
         ...
         <div className="flex flex-wrap gap-2 pt-2 border-t border-neutral-light flex-shrink-0">
          {[
            ...
            <button 
                ...
                title={section.title}
                className={`px-3 py-1.5 rounded-lg text-xs flex items-center shadow-sm hover:shadow-md transition-all ${section.show ? 'bg-primary text-white ring-1 ring-primary-dark' : 'bg-neutral-lightest hover:bg-neutral-lighter text-textSecondary'}`}
        ...
        {searchError && <p className="text-xs text-danger-dark text-center md:text-right flex-shrink-0">{searchError}</p>}
      </div>

      {/* Filter/Tool Controls */}
      {showPathfindingControls && ( <div className="p-2 bg-neutral-lightest/90 border border-neutral-light rounded-xl shadow-lg space-y-2 text-xs flex-shrink-0"> <h4 className="font-semibold text-textPrimary">Find Path Between Nodes</h4><div className="grid grid-cols-1 sm:grid-cols-2 gap-2 items-end"><input type="text" placeholder="Source Node ID" value={pathSourceNode} onChange={e => setPathSourceNode(e.target.value)} className="w-full p-1 border border-neutral-DEFAULT/50 rounded shadow-sm text-xs"/><input type="text" placeholder="Target Node ID" value={pathTargetNode} onChange={e => setPathTargetNode(e.target.value)} className="w-full p-1 border border-neutral-DEFAULT/50 rounded shadow-sm text-xs"/></div><div className="flex gap-2 mt-1"><button onClick={handleFindPath} className="px-2 py-1 bg-primary text-white rounded hover:bg-primary-dark text-xs shadow-sm flex items-center"><SearchIcon size={12} className="mr-1"/>Find</button><button onClick={handleClearPath} className="px-2 py-1 bg-neutral-light hover:bg-neutral-DEFAULT/30 text-textPrimary rounded text-xs shadow-sm flex items-center"><Eraser size={12} className="mr-1"/>Clear</button></div>{pathfindingError && <p className="text-xs text-danger-dark mt-1">{pathfindingError}</p>}</div> )}
      {fullTimeSpan && showTimelineControls && ( <div className="p-2 bg-neutral-lightest/90 border border-neutral-light rounded-xl shadow-lg space-y-2 text-xs flex-shrink-0"> <h4 className="font-semibold text-textPrimary">Filter by Timeline</h4><div className="space-y-1"><div className="flex items-center justify-between"><label htmlFor="timelineStart" className="text-textSecondary mr-2 text-[10px]">Start: {timelineFilter ? formatDateFromTimestamp(timelineFilter.start) : 'N/A'}</label><input type="range" id="timelineStart" min={fullTimeSpan.min} max={fullTimeSpan.max} value={timelineFilter?.start || fullTimeSpan.min} onChange={e => handleTimelineChange('start', e.target.value)} className="w-full accent-primary"/></div><div className="flex items-center justify-between"><label htmlFor="timelineEnd" className="text-textSecondary mr-2 text-[10px]">End: {timelineFilter ? formatDateFromTimestamp(timelineFilter.end) : 'N/A'}</label><input type="range" id="timelineEnd" min={fullTimeSpan.min} max={fullTimeSpan.max} value={timelineFilter?.end || fullTimeSpan.max} onChange={e => handleTimelineChange('end', e.target.value)} className="w-full accent-primary"/></div></div><button onClick={resetTimelineFilter} className="mt-1 px-2 py-1 bg-neutral-light hover:bg-neutral-DEFAULT/30 text-textPrimary rounded text-xs shadow-sm flex items-center"><Eraser size={12} className="mr-1"/>Reset</button></div> )}
      {showHighlightControls && ( <div className="p-2 bg-neutral-lightest/90 border border-neutral-light rounded-xl shadow-lg space-y-2 text-xs flex-shrink-0"> <h4 className="font-semibold text-textPrimary">Highlight Elements</h4><div className="grid grid-cols-1 sm:grid-cols-2 gap-2"><div className="space-y-1"><label className="block text-textSecondary text-[10px]">Usage Types:</label><div className="max-h-16 overflow-y-auto scrollbar-thin p-1 border rounded">{uniqueUsageTypesForFilter.map(type => <label key={type} className="flex items-center text-textSecondary text-[10px]"><input type="checkbox" name={type} checked={highlightFilters.usageTypes.includes(type)} onChange={e => handleHighlightFilterChange(e as any)} value={type} className="mr-1 h-3 w-3 accent-primary"/>{type}</label>)}</div></div><div className="space-y-1"><label className="block text-textSecondary text-[10px]">Edge Duration (s):</label><input type="number" name="minEdgeDuration" value={highlightFilters.minEdgeDuration ?? ''} onChange={handleHighlightFilterChange} placeholder="Min" className="w-full p-1 border rounded text-xs"/><input type="number" name="maxEdgeDuration" value={highlightFilters.maxEdgeDuration ?? ''} onChange={handleHighlightFilterChange} placeholder="Max" className="w-full p-1 border rounded text-xs mt-1"/></div></div><div className="flex items-center mt-1"><input type="checkbox" id="commonAcrossFiles" name="commonAcrossFiles" checked={highlightFilters.commonAcrossFiles} onChange={handleHighlightFilterChange} className="mr-1.5 h-3 w-3 accent-primary"/><label htmlFor="commonAcrossFiles" className="text-textSecondary text-[10px]">Nodes common across all files</label></div><div className="flex gap-2 mt-1"><button onClick={handleApplyHighlights} className="px-2 py-1 bg-primary text-white rounded hover:bg-primary-dark text-xs shadow-sm flex items-center"><Zap size={12} className="mr-1"/>Apply</button><button onClick={handleClearHighlights} className="px-2 py-1 bg-neutral-light hover:bg-neutral-DEFAULT/30 text-textPrimary rounded text-xs shadow-sm flex items-center"><Eraser size={12} className="mr-1"/>Clear</button></div>{highlightInteractionMessage && <p className="text-xs text-info-dark mt-1">{highlightInteractionMessage}</p>}</div> )}
      
      {/* Graph Area */}
      <div className="flex flex-grow min-h-0"> 
        <div 
          ref={graphContainerRef} 
          className="flex-grow border border-neutral-light rounded-xl bg-neutral-lightest/30 shadow-inner overflow-hidden" 
        />
        <SNAInsightsPanel 
      ...
      {nodeContextMenu.visible && nodeContextMenu.nodeId && !isPathfindingActive && !isHighlightActive && (
        <div style={{ top: nodeContextMenu.y, left: nodeContextMenu.x }} className="absolute z-50 bg-surface border border-neutral-light rounded-lg shadow-xl py-1.5 text-sm min-w-[240px]" onClick={(e) => e.stopPropagation()}>
            ...
            <>
                ...
                {nodePositions.has(nodeContextMenu.nodeId) && <button onClick={handleClearManualPosition} className="w-full text-left px-3 py-2 hover:bg-primary-lighter/50 flex items-center transition-colors text-xs"><Eraser size={14} className="mr-2 text-neutral-DEFAULT"/>Clear Manual Position</button>}
