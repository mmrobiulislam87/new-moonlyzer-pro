...
const IPDRFileUpload: React.FC = () => {
  const { addIPDRFile, removeIPDRFile, updateIPDRFileSourceName, uploadedIPDRFiles, setIsLoading, setError, error } = useIPDRContext();
    ...
    for (const file of acceptedFiles) {
      try {
        ...
        if (records.length === 0 && headers.length === 0) {
             console.warn(`File ${file.name} appears to be empty or not a valid IPDR format.`);
        }
  ...
  const saveSourceName = (fileId: string) => {
    if (currentEditValue.trim() !== "") updateIPDRFileSourceName(fileId, currentEditValue.trim());
  ...
  const startEditing = (file: UploadedIPDRFile) => { setEditingSourceNameId(file.id); setCurrentEditValue(file.sourceName); };
            ...
            {uploadedIPDRFiles.map((file) => (
              ...
              >
                <div className="flex items-center mb-2 sm:mb-0 flex-grow min-w-0">
                  <FileText className="h-6 w-6 text-accent mr-2.5 sm:mr-3 flex-shrink-0" />
                ...
                <div className="flex items-center space-x-2 flex-shrink-0 self-end sm:self-center mt-1 sm:mt-0">
                  ...
                  <button onClick={() => removeIPDRFile(file.id)} title="Remove file"
