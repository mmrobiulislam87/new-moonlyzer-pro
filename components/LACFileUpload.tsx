...
import { UploadedLACFile } from '../types';
import { parseLACExcelFile } from '../utils/lacParser';
...
const LACFileUpload: React.FC = () => {
  const { addLACFile, removeLACFile, updateLACFileSourceName, uploadedLACFiles, setIsLoading, setError, error } = useLACContext();
  ...
  const onDrop = useCallback(async (acceptedFiles: FileWithSourceName[]) => {
    ...
    for (const file of acceptedFiles) {
      try {
        const { records, headers } = await parseLACExcelFile(file);
  ...
  const saveSourceName = (fileId: string) => {
    if (currentEditValue.trim() !== "") updateLACFileSourceName(fileId, currentEditValue.trim());
  ...
  const startEditing = (file: UploadedLACFile) => { setEditingSourceNameId(file.id); setCurrentEditValue(file.sourceName); };
            ...
            {uploadedLACFiles.map((file) => (
              ...
              >
                <div className="flex items-center mb-2 sm:mb-0 flex-grow min-w-0">
                  <FileText className="h-6 w-6 text-info mr-2.5 sm:mr-3 flex-shrink-0" />
                ...
                <div className="flex items-center space-x-2 flex-shrink-0 self-end sm:self-center mt-1 sm:mt-0">
                  ...
                  <button onClick={() => removeLACFile(file.id)} title="Remove file"
