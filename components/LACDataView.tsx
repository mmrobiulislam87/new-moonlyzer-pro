...
import { ChevronDown, ChevronUp, FileText as FileIcon, Filter as FilterIcon, Download, Info, AlertTriangle, XCircle, Edit3, Check, X, Trash2, TowerControl, List, Loader2, Users2, Clock, Repeat, Smartphone, Route } from 'lucide-react';
      ...
      {filteredLACRecords.length === 0 && lacFilterState.searchTerm && uploadedLACFiles.length > 0 && (
       ...
       {filteredLACRecords.length === 0 && !lacFilterState.searchTerm && uploadedLACFiles.length > 0 && (
         <div className="p-4 bg-info-lighter border border-info-light rounded-lg text-center text-info-dark flex items-center shadow-md">
            <Info size={20} className="mr-2"/> No LAC/Cell records to display for the current selection.
         </div>
       )}
...
// New LACDataView component
const LACDataView: React.FC = () => {
    const { uploadedLACFiles, isLoading } = useLACContext();

    if (isLoading && uploadedLACFiles.length === 0) {
        return (
            <div className="flex justify-center items-center h-64">
                <Loader2 className="h-12 w-12 animate-spin text-info" />
                <p className="ml-3 text-textSecondary">Loading LAC/Cell Data...</p>
            </div>
        );
    }

    if (uploadedLACFiles.length === 0 && !isLoading) {
        return (
             <div className="flex items-center justify-center h-full">
                <div className="flex flex-col items-center justify-center text-center p-8 sm:p-12 bg-surface rounded-2xl shadow-xl border border-neutral-light/50 min-h-[450px] w-full max-w-4xl">
                    <TowerControl size={52} className="text-info mb-6"/>
                    <h2 className="text-2xl sm:text-3xl font-bold text-textPrimary mb-3">LAC & Cell Analysis Workspace</h2>
                    <p className="text-md sm:text-lg text-textSecondary mb-8 max-w-2xl">
                        Upload your Location Area Code (LAC) & Cell ID data to analyze device movements and co-location patterns. Uncover critical connections by tracking where and when devices were present.
                    </p>
                    <div className="flex flex-wrap justify-center gap-x-6 sm:gap-x-10 gap-y-4">
                        <div className="flex flex-col items-center text-xs sm:text-sm text-sky-500 w-24 text-center">
                            <><Users2 size={24} /><Clock size={24} className="-ml-2"/></>
                            <span className="mt-1">Same Time, Same Tower</span>
                        </div>
                        <div className="flex flex-col items-center text-xs sm:text-sm text-blue-500 w-24 text-center">
                           <Repeat size={32} className="mb-1" />
                           Frequent Presence
                        </div>
                        <div className="flex flex-col items-center text-xs sm:text-sm text-cyan-600 w-24 text-center">
                            <><Smartphone size={24} /><Users2 size={24} className="-ml-2"/></>
                            <span className="mt-1">Multi-SIM IMEI Linkage</span>
                        </div>
                        <div className="flex flex-col items-center text-xs sm:text-sm text-sky-700 w-24 text-center">
                           <Route size={32} className="mb-1" />
                           Tower Travel Patterns
                        </div>
                    </div>
                </div>
            </div>
