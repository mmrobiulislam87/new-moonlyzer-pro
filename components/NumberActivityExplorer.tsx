...
const NumberActivityExplorer: React.FC<NumberActivityExplorerProps> = ({ setActiveView }) => {
    ...
    (recordsForAnalysis || []).forEach(record => {
        ...
        if (aparty === number) { // Selected number is APARTY
            ...
            if (isCallInteraction) {
                ...
                } else if (isIncomingCallType(usageType)) { }
                if (hour !== -1) hourlyCallActivityArr[hour].count++;
            } else if (isSmsInteraction) {
  ...
  const summaryTitle = useMemo(() => {
    let title = "Activity Ranking Explorer";
    if (activeFileTabId) {
        const activeFile = uploadedFiles.find(f => f.id === activeFileTabId);
        if (activeFile) {
            title += ` for: ${activeFile.sourceName || activeFile.name}`;
        }
    } else if (filesToAnalyze.length > 0) {
        if (filesToAnalyze.length === 1) {
            title += ` for: ${filesToAnalyze[0].sourceName || filesToAnalyze[0].name}`;
        } else {
            title += ` (Across ${filesToAnalyze.length} Files)`;
        }
    } else if (uploadedFiles.length > 0) {
        title += ` (Across All Files)`;
    }
    return title;
      ...
      <div className="p-4 sm:p-5 bg-surface border border-neutral-light rounded-xl shadow-xl">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
            <div className="mb-2 md:mb-0">
                ...
                <TrendingUp size={24} className="mr-2.5 text-primary" /> {summaryTitle}
                </div>
