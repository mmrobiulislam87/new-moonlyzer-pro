...
const MAX_RECORDS_FOR_GRAPH = 50000;
const NODE_POSITIONS_STORAGE_KEY = 'zeniusMoonlyzerNodePositions';
...
export const CDRProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [nodePositions, setNodePositions] = useState<Map<string, NodePosition>>(() => {
    try {
      const storedPositions = localStorage.getItem(NODE_POSITIONS_STORAGE_KEY);
      if (storedPositions) {
        return new Map(JSON.parse(storedPositions));
      }
    } catch (e) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(NODE_POSITIONS_STORAGE_KEY, JSON.stringify(Array.from(nodePositions.entries())));
    } catch (e) {
      console.error("Failed to save node positions to localStorage", e);
    }
  }, [nodePositions]);

  const setNodePosition = useCallback((nodeId: string, position: NodePosition) => {
    setNodePositions(prev => new Map(prev).set(nodeId, position));
  }, []);

  const removeNodePosition = useCallback((nodeId: string) => {
  ...
  const { getTowerInfo, towerDatabase } = useLACContext(); 

  const enrichRecordWithTowerData = useCallback((record: CDRRecord): CDRRecord => {
    if (towerDatabase.length > 0 && record.LACSTARTA && record.CISTARTA) {
      const towerInfo = getTowerInfo(record.LACSTARTA, record.CISTARTA);
      if (towerInfo) {
  ...
  const hideEdge = useCallback((edgeIdToHide: string) => {
    const edgeToHide = graphData.edges.find(e => e.data.id === edgeIdToHide);
    if (!edgeToHide) return;

    const sourceId = edgeToHide.data.source;
    const targetId = edgeToHide.data.target;

    // The nodes to hide in this action, starting with the edge's nodes.
    const nodesToHideThisAction = new Set<string>([sourceId, targetId]);
    
    // Build an adjacency list of the currently visible graph.
    const visibleAdjList = new Map<string, Set<string>>();
    graphData.nodes.forEach(node => {
        if (!hiddenNodeIds.has(node.data.id)) {
            visibleAdjList.set(node.data.id, new Set<string>());
        }
    });

    graphData.edges.forEach(edge => {
        if (!hiddenEdgeIds.has(edge.data.id)) {
            const s = edge.data.source;
            const t = edge.data.target;
            // Only add connection if both nodes are currently visible
            if (visibleAdjList.has(s) && visibleAdjList.has(t)) {
                visibleAdjList.get(s)!.add(t);
                visibleAdjList.get(t)!.add(s);
            }
        }
    });

    // Find direct neighbors of source and target from the visible graph.
    const neighborsToInspect = new Set<string>();
    (visibleAdjList.get(sourceId) || []).forEach(neighbor => {
        if (neighbor !== targetId) neighborsToInspect.add(neighbor);
    });
    (visibleAdjList.get(targetId) || []).forEach(neighbor => {
        if (neighbor !== sourceId) neighborsToInspect.add(neighbor);
    });

    // Check if any of these neighbors will become isolates after hiding source and target.
    neighborsToInspect.forEach(neighborId => {
        const visibleNeighbors = visibleAdjList.get(neighborId);
        if (visibleNeighbors) {
            // Check if all visible neighbors of this node are about to be hidden.
            const willBeIsolated = Array.from(visibleNeighbors).every(n => nodesToHideThisAction.has(n));
            if (willBeIsolated) {
                nodesToHideThisAction.add(neighborId);
            }
        }
    });

    setHiddenNodeIds(prev => new Set([...prev, ...nodesToHideThisAction]));
    setHiddenEdgeIds(prev => new Set(prev).add(edgeIdToHide));
  }, [graphData, hiddenNodeIds, hiddenEdgeIds]);
  ...
  return (
    <CDRContext.Provider value={{ 
      ...
      allRecords,
      // `filteredRecords` is removed. All components will use `globallyFilteredRecords`.
      globallyFilteredRecords,
